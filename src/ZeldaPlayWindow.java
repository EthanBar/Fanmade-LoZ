...
public class ZeldaPlayWindow extends JFrame {
	...
	public ZeldaPlayWindow(String title) throws Exception {
		...
		this.setLocationRelativeTo(null); // Get that in the center
		...
		monLoad(); // That's a dumb name to load all the monsters.
		...
		try {
		    ...
		    e.printStackTrace(); // Prints the Stack Trace in case anything goes wrong. (It won't)
		}
		this.intro = false;
	}
    ...
    private Monster chooseMon(int monX, int monY, String theMonString, int theNum) throws IOException{
		...
		switch (theMonString) { // Takes the string of each monster and loads it hecka fast
			case ("redoctorok"): // Screw if/else loops
